...
namespace Dev4Press\v36\API;

use Dev4Press\v36\Core\Helpers\IP;
		...
		if ( ! is_null( $this->ad ) && $this->ad !== false && isset( $this->ad->plugins ) && $this->ad->plugins !== false && is_array( $this->ad->plugins ) && ! empty( $this->ad->plugins ) ) {
			...
			if ( $plugins ) {
				...
				foreach ( $plugins as $plugin ) {
					...
					$render .= '<a target="_blank" rel="noopener" href="' . $this->_url( $plugin->url_home, $panel ) . '"><i class="d4p-icon d4p-plugin-' . $plugin->code . '"></i></a>';
					$render .= '</div>';
