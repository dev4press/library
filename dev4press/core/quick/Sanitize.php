<?php
/**
 * Name:    Dev4Press\v50\Core\Quick\Sanitize
 * Version: v5.0
...
class Sanitize {
	...
	public static function deep( array $input, string $method, bool $strip_shortcodes = false ) : array {
		switch ( $method ) {
			default:
			case 'html':
				$input = map_deep( $input, '\Dev4Press\v50\Core\Quick\Sanitize::html' );
				break;
			case 'text':
				$input = map_deep( $input, '\Dev4Press\v50\Core\Quick\Sanitize::text' );
				break;
			case 'key':
				$input = map_deep( $input, '\Dev4Press\v50\Core\Quick\Sanitize::key' );
				break;
			case 'slug':
				$input = map_deep( $input, '\Dev4Press\v50\Core\Quick\Sanitize::slug' );
				break;
		}
